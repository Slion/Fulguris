...
### ⚠️ CRITICAL: PowerShell Double Quote Issue with XML

**NEVER use outer double quotes with --set-raw when the value contains XML with escaped quotes!**

❌ **WRONG - This CORRUPTS the XML:**
```powershell
python l10n.py --set-raw el-rGR message_ssl_error "text <xliff:g id=\"domain\">%s</xliff:g> more"
# Result: The \" inside gets mangled by PowerShell's quote processing
```

✅ **CORRECT - Use SINGLE quotes for the outer string:**
```powershell
python l10n.py --set-raw el-rGR message_ssl_error 'text <xliff:g id="domain">%s</xliff:g> more'
# Single quotes prevent PowerShell from processing the inner quotes
```

**Why this happens:**
- PowerShell processes escape sequences inside double quotes differently
- The `\"` meant for the XML gets corrupted by PowerShell's own escaping rules
- Using single quotes (`'...'`) tells PowerShell to treat everything literally
- This is especially critical for `--set-raw` and `--set-plurals` commands with XML content

**Always use single quotes when:**
- Setting strings with `<xliff:g>` tags
- Using `--set-raw` or `--set-plurals`
- The value contains any quotes that need to be preserved
