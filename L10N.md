...
### ⚠️ CRITICAL: PowerShell XML Quote Escaping for --set-raw
...
✅ **CORRECT - Escape inner quotes with backtick or use here-string:**

**Method 1: Escape quotes with backtick (`) inside single quotes:**
```powershell
python l10n.py --set-raw el-rGR message_session_imported 'Text <xliff:g id=`"session_name`" example=`"News`">%s</xliff:g>'
```

**Method 2: Use PowerShell here-string (BEST for complex XML):**
```powershell
$value = @'
Text <xliff:g id="session_name" example="News">%s</xliff:g>
'@
python l10n.py --set-raw el-rGR message_session_imported $value
```

**Method 3: Edit XML file directly for complex strings**
For strings with quotes inside XML tags, it's often easier and safer to edit the file directly.

❌ **WRONG - These all break:**
```powershell
# Double quotes - PowerShell processes \" incorrectly
python l10n.py --set-raw el-rGR test "text <xliff:g id=\"x\">%s</xliff:g>"

# Single quotes - PowerShell strips the attribute quotes!
python l10n.py --set-raw el-rGR test 'text <xliff:g id="x">%s</xliff:g>'
# Result: <xliff:g id=x> (missing quotes = XML error)
```
...
**Best practice for --set-raw with XML:**
1. Use PowerShell here-strings for complex XML
2. OR escape each quote with backtick: `"
3. OR edit the XML file directly for maximum safety
