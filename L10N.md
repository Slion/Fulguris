...
# For complex XML strings, use --raw flag with here-strings
$value = @'
...
✅ **ALTERNATIVE - For simple strings without XML tags:**
```powershell
...
1. **Use PowerShell here-strings** (the `@'...'@` syntax) with `--raw` flag - most reliable
2. **Use regular `--set`** for simple strings without XML tags - automatic escaping
