...
- **Controlled String Addition**: Use `--add` flag to explicitly add new strings (safer default)
...
**⚠️ IMPORTANT: Adding New Strings**

By default, the tool will **reject** attempts to set strings that don't exist in the target language file. You must use the `--add` flag to explicitly allow adding new strings.

**Why this matters:** Only translate strings that exist in the English base file (`values/strings.xml`). Adding non-existent strings creates unused entries that need cleanup later.

**Single String (Existing):**
```bash
python l10n.py --set ko-rKR settings_title '설정'
# Updates existing string, or errors if it doesn't exist
```

**Single String (Adding New):**
```bash
python l10n.py --add --set ko-rKR settings_title '설정'
# Adds the string if it doesn't exist, updates if it does
# Only use --add if you've verified the string exists in values/strings.xml!
```
