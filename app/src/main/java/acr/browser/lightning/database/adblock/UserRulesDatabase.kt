...
import acr.browser.lightning.database.adblock.UserRulesRepository.Companion.RESPONSE_BLOCK
import acr.browser.lightning.database.adblock.UserRulesRepository.Companion.RESPONSE_EXCLUSION
import acr.browser.lightning.database.adblock.UserRulesRepository.Companion.RESPONSE_NOOP
...
class UserRulesDatabase @Inject constructor(
...
) : SQLiteOpenHelper(application, DATABASE_NAME, null, DATABASE_VERSION), UserRulesRepository {
    ...
    private fun UnifiedFilterResponse.toContentValues() = ContentValues(10).apply {
        put(KEY_RESPONSE, response.toInt())
        put(KEY_PATTERN, filter.pattern)
        put(KEY_FILTER_TYPE, filter.filterType)
        put(KEY_CONTENT_TYPE, filter.contentType)
        put(KEY_THIRD_PARTY, filter.thirdParty)
        put(KEY_DOMAIN_MAP, filter.domains?.toDBString() ?: "")
    }
