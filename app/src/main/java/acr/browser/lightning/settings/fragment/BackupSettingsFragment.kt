...
class BackupSettingsFragment : AbstractSettingsFragment() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        // Bookmarks
        clickablePreference(preference = SETTINGS_BOOKMARKS_EXPORT, onClick = this::exportBookmarks)
        clickablePreference(preference = SETTINGS_BOOKMARKS_IMPORT, onClick = this::importBookmarks)
    ...
    private fun loadFileList(path: File?): Array<File> {
        ...
        try {
            ...
            logger.log(BOOKMARK_TAG, "Unable to make directory", e)
        }
                        ...
                        .subscribe { list ->
                            ...
                            exportSubscription = BookmarkExporter.exportBookmarksToFile(list, outputStream)
                                ...
                                .subscribeBy(
                                    ...
                                    onError = { throwable ->
                                        logger.log(BOOKMARK_TAG, "onError: exporting bookmarks", throwable)
                                        val activity = activity
                                        if (activity != null && !activity.isFinishing && isAdded) {
                                            Utils.createInformativeDialog(activity, R.string.title_error, R.string.bookmark_export_failure)
                                        } else {
            ...
            result.data?.data?.let{ uri ->
                context?.contentResolver?.openInputStream(uri).let { inputStream ->
                    ...
                    .subscribeBy(
                        ...
                        onError = {
                            logger.log(BOOKMARK_TAG, "onError: importing bookmarks", it)
                            val activity = activity
                            if (activity != null && !activity.isFinishing && isAdded) {
        ...
        PermissionsManager.getInstance().requestPermissionsIfNecessaryForResult(activity, REQUIRED_PERMISSIONS,
                object : PermissionsResultAction() {
                    ...
                    override fun onDenied(permission: String) {
                        val activity = activity
                        if (activity != null && !activity.isFinishing && isAdded) {
                            Utils.createInformativeDialog(activity, R.string.title_error, R.string.history_export_failure)
                        } else {
                            application.toast(R.string.history_export_failure)
                        }
                    }
                })
    }

    private fun showExportHistoryDialog() {
        //TODO: specify default path
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            addCategory(Intent.CATEGORY_OPENABLE)
            type = "text/plain" // Specify type of newly created document
    ...
    //
    private val historyExportFilePicker = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        result: ActivityResult ->
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            result.data?.data?.let{ uri ->
                context?.contentResolver?.openOutputStream(uri)?.let { outputStream ->
                    ...
                    historySubscription = historyRepository.getAllHistoryEntriesAsSingle()
                            .subscribeOn(databaseScheduler)
                            .subscribe { list ->
                                if (!isAdded) {
                                    return@subscribe
                                ...
                                exportSubscription?.dispose()
                                exportSubscription = HistoryExporter.exportHistoryToFile(list, outputStream)
                                        .subscribeOn(databaseScheduler)
                                        .observeOn(mainScheduler)
                                        .subscribeBy(
                                                onComplete = {
                                                    activity?.apply {
                                                        snackbar("${getString(R.string.history_export_path)} ${uri.fileName}")
                                                    }
                                                },
                                                onError = { throwable ->
                                                    logger.log(HISTORY_TAG, "onError: exporting history", throwable)
                                                    val activity = activity
                                                    if (activity != null && !activity.isFinishing && isAdded) {
                                                        Utils.createInformativeDialog(activity, R.string.title_error, R.string.history_export_failure)
                                                    } else {
    ...
    companion object {
        ...
        private const val BOOKMARK_TAG = "BookmarkSettingsFrag"
        private const val HISTORY_TAG = "HistorySettingsFrag"

        private const val SETTINGS_BOOKMARKS_EXPORT = "export_bookmark"
        private const val SETTINGS_BOOKMARKS_IMPORT = "import_bookmark"
